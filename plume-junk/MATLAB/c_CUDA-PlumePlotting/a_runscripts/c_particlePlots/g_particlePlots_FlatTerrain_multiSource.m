...
codeInputDir = sprintf("%s/../../../../../testCases/FlatTerrain/c_plumeOutputs/multiSource",workingDir);
caseBaseName = "FlatTerrain_multiSource";
folderFileNames = [
...
plotOutputDir = sprintf("%s/../../../../../testCases/FlatTerrain/e_matlabPlotOutput/multiSource/c_particlePlots",workingDir);
...
nSources = max(max(lagr_sourceIdx));
nx = length(urb_x);
ny = length(urb_y);
nz = length(urb_z);
...
for timeIdx = 1:nTimes
    ...
    current_sourceIdx = lagr_sourceIdx(isActive_indices,1);
    ...
    % create the legend string to hold the initial positions and all the
    % source indices
    legendString = strings(nSources+1,1);
    legendString(1) = "initial locations";
    
    
    % sort the current positions by the sourceIdx
    dataToSort = [current_sourceIdx,current_xPos,current_yPos,current_zPos];
    [~,foundIndices] = sort(dataToSort(:,1)); % sort just the first column
    sortedData = dataToSort(foundIndices,:);   % sort the whole matrix using the sort indices
    current_sourceIdx = sortedData(:,1);
    current_xPos = sortedData(:,2);
    current_yPos = sortedData(:,3);
    current_zPos = sortedData(:,4);
    
    
    % now plot the current positions
    hold on;
    for sourceIdx = 0:nSources
        source_indices = find(current_sourceIdx == sourceIdx);
        scatter3(current_xPos(source_indices),current_yPos(source_indices),current_zPos(source_indices),'+');
        legendString(sourceIdx+2) = sprintf("source %d",sourceIdx);
    end
