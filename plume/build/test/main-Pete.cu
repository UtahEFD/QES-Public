...
//   thrust::host_vector<Cell> h_ptr(nx*ny*nz);// = thrust::host_malloc<Cell>(10);
  thrust::device_vector<Cell> d_ptr = thrust::device_malloc<Cell>(nx*ny*nz);
...
// 	QUICWindField >> ((Cell)(d_ptr[kk])).wind.x;//cell.wind.x;////h_ptr[p2idx].wind.x;//windFieldData[p2idx].u;
	//QUICWindField >> d_ptr[kk].wind.x;//cell.wind.y;//h_ptr[p2idx].wind.y;//windFieldData[p2idx].v;
	//QUICWindField >> d_ptr[kk].wind.x;//c/ell.wind.z;//h_ptr[p2idx].wind.z;//windFieldData[p2idx].w;
	thrust::fill(d_ptr.begin()+kk-1, d_ptr.begin() + kk, cell);
...
// 		  << windFieldData[p2idx].w << ")" << std::endl;
  ...
  {
//     std::cout << "  wind.x  " <<d_ptr[k].wind.x
// 	      << "  wind.y  " <<d_ptr[k].wind.y
// 	      << "  wind.z  " <<d_ptr[k].wind.z
// 	      <<std::endl;
  }
