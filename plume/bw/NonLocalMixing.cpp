...
void nonLocalMixing::createSigTau(eulerian* eul,util& utl){

    double sigUOrg   = 1.8;
    double sigVOrg   = 2.0;
    double sigWOrg   = 1.3;
    double sigUConst = sigUOrg;
    double sigVConst = sigVOrg;
    double sigWConst = sigWOrg;
    
    std::cout<<"IN Non-Local Mixing Func"<<std::endl;
    ...
    double TvMax=*std::max_element(Tv.begin(),Tv.end());
    double ThMax=*std::max_element(Th.begin(),Th.end());
    double sumThTvMax=TvMax+ThMax;
        ...
        for(int k = 0; k < kEnd;k++){
            for(int j = jStrt; j < jEnd;j++){
                for(int i = iStrt; i < iEnd;i++ ){
                    ...
                    int id = k*ny*nx+j*nx+i;
                    if(eul->CellType.at(id).c!=0){
                        Tv.at(id) = Tv.at(id)/sumThTvMax;
                        Th.at(id) = Th.at(id)/sumThTvMax;
                    }
                }
            }
        }
        ...
        for(int k = 0; k < kEnd;k++){
            for(int j = jStrt; j < jEnd;j++){
                for(int i = iStrt; i < iEnd;i++ ){
                    ...
                    int id        = k*ny*nx+j*nx+i;
                    if(eul->CellType.at(id).c!=0){
                        int idCenLine = int(k*nx*ny+yfo.at(ibld)*nx+i);
                        int idTop     = (kEnd+1)*nx*ny+j*nx+i;
                        int idRefHor  = k*nx*ny+(jEnd+1)*nx+i;
                        
                        double URefHor = eul->windVec.at(idRefHor ).u;
                        double ustarH  = Ch*Th.at(id)*fabs(URefHor-eul->windVec.at(idCenLine ).u);
                        double ustarV  = Cv*Tv.at(id)*fabs(eul->windVec.at(idTop ).u);
                        double Lm,ustar;
                        if(ustarH>ustarV){
                            ustar=ustarH;
                            Lm=wth.at(ibld);
                        ...
                        eul->tau.at(id).e11 = sigUConst*sigUConst*ustar2;
                        eul->tau.at(id).e22 = sigVConst*sigVConst*ustar2;
                        eul->tau.at(id).e33 = sigWConst*sigWConst*ustar2;
                        eul->tau.at(id).e12 = sigUConst*sigVConst*ustar2;
                        eul->tau.at(id).e23 = sigVConst*sigWConst*ustar2;
                        eul->tau.at(id).e13 =-fabs(sigUConst*sigWConst*ustar2);
                        eul->lam.at(id)=eul->matrixInv(eul->tau.at(id));
                        
                        eul->sig.at(id).e11 = sigUConst*ustar;
                        eul->sig.at(id).e22 = sigVConst*ustar;
                        eul->sig.at(id).e33 = sigWConst*ustar;
                        eul->CoEps.at(id)=5.7* pow(ustar,3.0)/(Lm);
                    }
                }
            }
        }
	...
	if(eul->CellType.at(id).c!=0){ //Calculate gradients ONLY if it is a fluid cell    
	  ...
	  double Tke  = 0.0;
          ...
          if(i>0 && i<nx-1 && j>0 && j<ny-1 && k>0 && k<nz-1){
              ...
              if(i>=xfo.at(3) && i<xfo.at(8) && k<=hgt.at(3)+5){
                  
                  // For wind gradients in X-direction
                  minDistance=hgt.at(3);
                  double dUdx=0.0;
                  double dVdx=0.0;
                  double dWdx=0.0;
                  ...
                  if(j==0 || j==ny){//boundaries
                      ...
                      int idYRef=  k*ny*nx + (yfo.at(3)+wth.at(3)/2+3)*nx + i;
                      dUdy = (eul->windVec.at(idYRef).u - eul->windVec.at(id).u);
                      dVdy = (eul->windVec.at(idYRef).v - eul->windVec.at(id).v);
                      dWdy = (eul->windVec.at(idYRef).w - eul->windVec.at(id).w);
                  }
                  ...
                  if(k==0){//ground
                      ...
                      int idZRef=  (hgt.at(3)+3)*ny*nx + j*nx + i;
                      dUdz= ( eul->windVec.at(idZRef).u-eul->windVec.at(id).u );
                      dVdz= ( eul->windVec.at(idZRef).v-eul->windVec.at(id).v );
                      dWdz= ( eul->windVec.at(idZRef).w-eul->windVec.at(id).w );
                  }
              }//if non-localmixing
...
double nonLocalMixing::getMinDistance(int i, int j, int qk){
