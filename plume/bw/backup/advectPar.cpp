...
void advectPar(const util &utl, dispersion &disp,eulerian &eul, const char* model, const int argc){
  ...
  std::cout<<"advectPar start...\n"<<nx<<" "<<ny<<" "<<nz<<" \n"
	   <<numBoxX<<" "<<numBoxY<<" "<<numBoxZ<<" \n";
  ...
  lBndz=utl.bnds[4];
  uBndz=utl.bnds[5];
// lBndx33 lBndy0 lBndz0 
// uBndx153 uBndy110 uBndz30 
...
//     if(xPos!=43.6667||yPos!=55||xPos!=0.666667)
//     std::cout<<" xPos"<<xPos<<" yPos"<<yPos<<" zPos"<<zPos<<"\n";
  }
//   for(int i=0; i<disp.prime.size(); i++)
//   {
//      std::cout<<i<<" disp.prime.at(i).x:"<<disp.prime.at(i).x<<" disp.prime.at(i).y:"
//      <<disp.prime.at(i).y<<" disp.prime.at(i).z:"<<disp.prime.at(i).z<<"\n";
//   }
// return;
////numTimeStep=1000
  //For every time step
  for(tStep=0; tStep<numTimeStep; tStep++){
    ...
    for(int par=0; par<parToMove;par++){
      ...
      int count=0;
      int countPrm=0;
      //xPos43.6667 yPos55 zPos0.666667 point emmiter
      ...
      double tStepRem=tStepInp;//tStepInp is util timestep
      double tStepUsed=0.0;
//   for(int i=0; i<disp.prime.size(); i++)
//   {
//      std::cout<<" disp.prime.at(i).x:"<<disp.prime.at(i).x<<" disp.prime.at(i).x:"
//      <<disp.prime.at(i).y<<" disp.prime.at(i).x:"<<disp.prime.at(i).z<<"\n";
//   }
	...
	if(iV>0 && iV<nx-1 && jV>0 && jV<ny-1 && kV>0 && kV<nz-1 && eul.CellType.at(id).c!=0){
	    ...
	    tStepCal=tFac * min(tStepArr,4); //compare array to get minium value
	    double arrT[]={tStepMin,tStepCal,tStepRem,dt};
	    dt=min(arrT,4);//compare array to get minium value
	    double uPrime=disp.prime.at(par).x;
	    double vPrime=disp.prime.at(par).y;
	    double wPrime=disp.prime.at(par).z; 
//   std::cout<<"uPrime"<<uPrime<<" vPrime"<<vPrime<<" wPrime"<<wPrime<<std::endl;
  std::cout<<"tStepSigW "<<tStepSigW <<std::endl;
  //std::cin.ignore();
	    ...
	    std::cout<<" ranU "<<ranU<<" ranV "<<ranV<<" ranW "<<ranW<<"\n";
...
// 	         std::cout<<id<<"URot:  "<<URot<<"VRot:  "<<VRot<<"WRot:  "<<WRot<<"\n" ;
// 	         std::cout<<id<<"ka0_11:  "<<ka0_11<<"ka0_21:  "<<ka0_21<<"ka0_31:  "<<ka0_31<<"\n" ;
// 	         std::cout<<id<<"eigVal_11:  "<<eigVal_11<<"eigVal_22:  "<<eigVal_22<<"eigVal_33:  "<<eigVal_33<<"\n" ;
// 	          std::cout<<id<<dt<<"\n" ;
...
/*	      	      
std::cout<<id<<"e11:  "<<eul.eigVec.at(id).e11<<"e12:  "<<eul.eigVec.at(id).e12<<"e13:  "<<eul.eigVec.at(id).e13<<"\n" ;
std::cout<<id<<"e21:  "<<eul.eigVec.at(id).e21<<"e22:  "<<eul.eigVec.at(id).e22<<"e23:  "<<eul.eigVec.at(id).e23<<"\n" ;
std::cout<<id<<"e31:  "<<eul.eigVec.at(id).e31<<"e32:  "<<eul.eigVec.at(id).e32<<"e33:  "<<eul.eigVec.at(id).e33<<"\n" ;
 	 */  
	      flag_g2nd=0;
		...
		double quan3=(1.0-g2nd_31*W_1st*dt);
	      std::cout<< "g2nd_11*U_1st: "<<g2nd_11*U_1st << " g2nd_11: "<<g2nd_11 << " U_1st: "<<U_1st <<"\n";
		...
		if(g2nd_11*U_1st!=0.0 && count<countMax){
	      std::cout<< "g2nd_11*U_1st!=0.0 " <<"\n";
	      std::cout<< "quan1: "<<quan1 <<"\n";
		  if(fabs(quan1)<0.5){
		    tStepMin=2.0*dt;
		    count++;
		    continue;
		...
		if(g2nd_21*V_1st!=0.0 && count<countMax){
	      std::cout<< "g2nd_21*V_1st!=0.0" <<"\n";
	      std::cout<< "quan2: "<<quan2 <<"\n";
		  if(fabs(quan2)<0.5){
		    tStepMin=2.0*dt;
		    count++;
		    continue;
		...
		if(g2nd_31*W_1st!=0.0 && count<countMax){
	      std::cout<< "g2nd_31*W_1st!=0.0" <<"\n";
	      std::cout<< "quan3: "<<quan3 <<"\n";
		  if(fabs(quan3)<0.5){
		    tStepMin=dt*2.0;
		    count++;
		    continue;
		...
		std::cout<<"Passed the test test test test test"<<g2nd_31<<"   "<<W_1st<<std::endl;
...
// 	      std::cout<< "U_2nd: "<<U_2nd << " V_2nd: "<<V_2nd << " W_2nd: "<<W_2nd <<"\n"; 
	      ...
	      vPrime=V_2nd+dv_3rd;
	      wPrime=W_2nd+dw_3rd;
	      std::cout<< "uPrime: "<<uPrime << " vPrime: "<<vPrime << " wPrime: "<<wPrime <<"\n"; 
