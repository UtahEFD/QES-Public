...
//   thrust::device_ptr<uint>   dev_concensPtr(concens); 
  thrust::device_ptr<bool> dev_seed_flag_ptr(seeds_flag);    
  
  thrust::device_vector<float4> devDebug(numParticles); 

  timeval t;
  gettimeofday(&t,0);
  thrust::for_each(
      thrust::make_zip_iterator(
	thrust::make_tuple(dev_pos4, dev_winP4, dev_seed_flag_ptr, devDebug.begin()) 
      ), 
      thrust::make_zip_iterator(
	thrust::make_tuple(dev_pos4+numParticles, dev_winP4+numParticles, dev_seed_flag_ptr+numParticles, devDebug.end())
     ), 
...
{   
  ...
  concentration_kernel<<< numBlocks, numThreads >>>(thrust::raw_pointer_cast(d_pos4),
						    thrust::raw_pointer_cast(d_concensPtr));
  
  random_file.open ("cal_concentration.txt", std::ios::out);
