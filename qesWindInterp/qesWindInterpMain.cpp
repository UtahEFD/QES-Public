...
int main(int argc, char *argv[])
{
    ...
    WINDSInputData* WID = nullptr;
    if (arguments.quicFile != "") {
        WID = new WINDSInputData(arguments.quicFile);
    }
    ...
    */
    ...
    WINDSGeneralData* WGD = nullptr;
    if ( WID ) {
        WGD = new WINDSGeneralData(WID, arguments.solveType);
    } else {
        WID = new WINDSInputData();
        WID->simParams = new SimulationParameters();
        WID->fileOptions = new FileOptions();
        WID->fileOptions->outputFields.push_back("all");
        WGD = new WINDSGeneralData(arguments.inWKFile);
    }
    ...
    */
    ...
    for (int index = 0; index < WGD->timestamp.size(); index++) {
