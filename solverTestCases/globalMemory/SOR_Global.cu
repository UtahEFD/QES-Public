...
__global__ void SOR_RB(double *d_lambda, int nx, int ny, int nz, float omega, float  A, float  B, float  dx, float *d_e, float *d_f, float *d_g, float *d_h, float *d_m, float *d_n, float *d_o, float *d_p, float *d_q, double *d_R, int offset){
    ...
    if ( (i > 0) && (i < nx-1) && (j > 0) && (j < ny-1) && (k < nz-1) && ((i+j+k)%2) == offset ){
              ...
              d_lambda[ii] = (omega/(2*(d_o[ii] + A*d_p[ii] + B*d_q[ii]))) * ((-1*(pow(dx, 2.0))*d_R[ii])+d_e[ii]*d_lambda[ii+1]+d_f[ii]*d_lambda[ii-1]+A*d_g[ii]*d_lambda[ii + nx]                               +A*d_h[ii]*d_lambda[ii - nx]+B*d_m[ii]*d_lambda[ii + nx*ny]+B*d_n[ii]*d_lambda[ii - nx*ny])+(1-omega)*d_lambda[ii];    // SOR formulation 
    }
    }
...
int main(int argc, const char * argv[]) {
	...
	SOR_RB<<<numberOfBlocks,numberOfThreadsPerBlock,0>>>(d_lambda, nx, ny, nz, omega, A, B, dx, d_e, d_f, d_g, d_h, d_m, d_n, d_o, d_p, d_q, d_R,offset);
    	cudaCheck(cudaGetLastError());    
	...
	// Invoke red-black SOR kernel
       ...
       // cudaFree (d_lambda);
        ...
        // Neumann boundary condition (p1 (@k=0) = p1 (@k=1))
        for (int j = 0; j < ny; j++){
            for (int i = 0; i < nx; i++){
                int ii = i + j*nx;          // Lineralize the vectors (make it 1D)
