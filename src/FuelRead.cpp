...
FuelRead::FuelRead(const std::string &filename, double cellSizeXN, double cellSizeYN)
...
FuelRead::FuelRead(const std::vector<double> &heightField, int dimX, int dimY, double cellSizeXN, double cellSizeYN)
{
    Triangle *tPtr=0;
    m_triList.clear();
    ...
    std::cout << "Fuel Loading from height field\n";
    std::cout << "dimX = " << dimX << ", dimY = " << dimY << std::endl;
    std::cout << "size of fuelField = " << heightField.size() << std::endl;
...
void FuelRead::load()
{
  std::cout << "load: loading Fuels..." << std::endl;
  ...
  if (rasterErr == CE_Failure) {
      std::cerr << "CPL RasterIO failure during Fuel loading. Exiting." << std::endl;
      exit(EXIT_FAILURE);
  }
...
void FuelRead::setDomain(Vector3<int>* domain, Vector3<float>* grid)
{
    for (int i = 0; i < 3; i++)
    {
      min[i] = LIMIT;
...
void FuelRead::outputOBJ(std::string s)
{
  std::ofstream file;
  file.open(s.c_str());
...
void FuelRead::printProgress (float percentage)
{
...
std::vector<int> FuelRead::setCells(Cell* cells, int nx, int ny, int nz, float dx, float dy, std::vector<float> &dz_array, std::vector<float> z_face, float halo_x, float halo_y) const
{
...
void FuelRead::setCellPoints(Cell* cells, int i, int j, int nx, int ny, int nz, std::vector<float> &dz_array, std::vector<float> z_face, Vector3<float> corners[], std::vector<int>& cutCells) const
{
...
Vector3<float> FuelRead::getIntermediate(Vector3<float> a, Vector3<float> b, float height) const
