#ifndef __FUEL_READ_H__
#define __FUEL_READ_H__ 1
...
public:
  ...
  friend class test_FuelRead;

  FuelRead();
  FuelRead(const std::string &filename, double cellSizeXN, double cellSizeYN);

    FuelRead(const std::vector<double> &heightField, int dimX, int dimY, double cellSizeXN, double cellSizeYN);
