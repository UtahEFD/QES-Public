    ...
    if (WID->metParams->sensors[i]->TS[index]->site_blayer_flag == 4) {
      ...
      for (auto k = WGD->terrain_id[site_id[i]]; k < WGD->nz - 1; k++) {
        if ((WGD->z[k] - WGD->z[WGD->terrain_id[site_id[i]] - 1]) < WID->metParams->sensors[i]->TS[index]->site_z_ref[0] || z_size == 1) {
          ...
          site_theta[i] = (270.0 - (WID->metParams->sensors[i]->TS[index]->site_wind_dir[ii] + wind_dir * ((WGD->z[k] - WGD->z[WGD->terrain_id[site_id[i]] - 1]) - WID->metParams->sensors[i]->TS[index]->site_z_ref[ii]))) * M_PI / 180.0;
  ...
  for (auto j = 0; j < WGD->ny; j++) {
    for (auto i = 0; i < WGD->nx; i++) {
      ...
      for (auto ii = 0; ii < num_sites; ii++) {
        ...
        wms[ii][i][j] = exp((-1 / (s_gamma * lamda)) * pow(WID->metParams->sensors[ii]->site_xcoord - x[i], 2.0) - (1 / (s_gamma * lamda)) * pow(WID->metParams->sensors[ii]->site_ycoord - y[j], 2.0));
        sum_wm += wm[ii][i][j];
      }
      if (sum_wm == 0) {
        for (auto ii = 0; ii < num_sites; ii++) {
