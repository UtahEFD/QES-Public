...
void TURBWall::set_loglaw_stairstep_at_id_cc(WINDSGeneralData *WGD,TURBGeneralData *TGD,
                                             int id_cc,int flag2check,float z0)
{
    ...
    if (WGD->icellflag[id_cc-1]==flag2check) {
        ...
        idm = id_fc;   //i,j,k
        vp=((TGD->y_cc[j]-TGD->y_fc[j])*WGD->v[idp]+
            (TGD->y_fc[j+1]-TGD->y_cc[j])*WGD->v[idm])/(TGD->y_fc[j+1]-TGD->y_fc[j]);
        ...
        wp=((TGD->z_cc[k]-TGD->z_fc[k])*WGD->w[idp]+
            (TGD->z_fc[k+1]-TGD->z_cc[k])*WGD->w[idm])/(TGD->z_fc[k+1]-TGD->z_fc[k]);
        ...
        idp = id_fc-1+nx; //i-1,j+1,k
        idm = id_fc-1;   //i-1,j,k
        vm=((TGD->y_cc[j]-TGD->y_fc[j])*WGD->v[idp]+
            (TGD->y_fc[j+1]-TGD->y_cc[j])*WGD->v[idm])/(TGD->y_fc[j+1]-TGD->y_fc[j]);
        // ==>
    ...
    if (WGD->icellflag[id_cc-(nx-1)]==flag2check) {
        ...
        idp = id_fc+1; //i+1,j,k
        idm = id_fc;   //i,j,k
        up=((TGD->x_cc[i]-TGD->x_fc[i])*WGD->u[idp]+
            (TGD->x_fc[i+1]-TGD->x_cc[i])*WGD->u[idm])/(TGD->x_fc[i+1]-TGD->x_fc[i]);
        ...
        idp = id_fc+1-nx; //i+1,j-1,k
        idm = id_fc-nx;   //i,j-1,k
        um=((TGD->x_cc[i]-TGD->x_fc[i])*WGD->u[idp]+
            (TGD->x_fc[i+1]-TGD->x_cc[i])*WGD->u[idm])/(TGD->x_fc[i+1]-TGD->x_fc[i]);
        // ==>
        ...
        idm = id_fc+nx;   //i,j,+1,k
        wp=((TGD->z_cc[k-1]-TGD->z_fc[k])*WGD->w[idp]+
            (TGD->z_fc[k+1]-TGD->z_cc[k])*WGD->w[idm])/(TGD->z_fc[k+1]-TGD->z_fc[k]);
        // w_hat-
        idp = id_fc-nx+nx*ny; //i,j-1,k+1
        idm = id_fc-nx;   //i,j-1,k
