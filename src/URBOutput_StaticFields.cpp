...
#include "UrbOutput_StaticFields.h"

URBOutput_StaticFields::URBOutput_StaticFields(URBGeneralData *ugd)
  : URBOutput_Generic(ugb), NetCDFOutput(...)
{
  ...
  // create attributes
  // AttScalarDbl att_t = {&(ugd->time),"t", "time","s", dim_scalar_t};
  AttVectorDbl att_x = {&(ugd->x_out), "x", "x-distance", "m", dim_scalar_x};
  AttVectorDbl att_y = {&(ugd->y_out), "y", "y-distance", "m", dim_scalar_y};
  AttVectorDbl att_z = {&(ugd->z_out), "z", "z-distance", "m", dim_scalar_z};
  ...
  AttVectorDbl att_h = {&(ugd->terrain),
  ...
  /*
    FM -> adjust as needed for other static fields
...
/*
  FM -> might not be needed here
  bool URBOutput_St::validateFileOtions()
  {
...
void URBOutput_StaticFields::save()
{
  int nx = ugd->nx;
  int ny = ugd->ny;
  int nz = ugd->nz;
    
  // FM -> need to clean (eg vector_size not used)
  ...
  // FM -> x_out,y_out and z_out should be computed here??

  
  /*
    FM -> not needed here (TO REMOVE)
  // set time
  ...
  */    
  ...
  // FM -> not needed as time is not save in static fields
  ...
  for (int i=0; i<output_vector_dbl.size(); i++) {
    ...
    if (i<3 && output_counter==0) {
      ...
      // FM -> not needed as no 3D fields as statis ??
      //} else {
  ...
  // FM -> not needed icell not static
