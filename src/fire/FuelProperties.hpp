...
public:
  ...
  FuelProperties(float oneHour, float tenHour, float hundredHour, float liveHerb, float liveWoody, float savOneHour, float savHerb, float savWoody, float fuelDepth, float heatContent, float fuelDensity, float fuelmce) : oneHour(oneHour), tenHour(tenHour), hundredHour(hundredHour), liveHerb(liveHerb), liveWoody(liveWoody),
