...
public:
	...
	template <typename X> Matrix3(const X a11, const X a12, const X a13,
                                  const X a21, const X a22, const X a23,
                                  const X a31, const X a32, const X a33) {
        m_val[0]=a11; m_val[1]=a12; m_val[2]=a13;
        m_val[3]=a21; m_val[4]=a22; m_val[5]=a23;
        m_val[6]=a31; m_val[7]=a32; m_val[6]=a33;
    }
	...
	bool operator==(const Matrix3<T>& v) {
		if (std::is_same<T,float>::value || std::is_same<T,double>::value)
			...
			return v.m_val[0] == m_val[0] && v.m_val[1] == m_val[1] && v.m_val[2] == m_val[2] &&
                v.m_val[3] == m_val[3] && v.m_val[4] == m_val[4] && v.m_val[5] == m_val[5] &&
                v.m_val[6] == m_val[6] && v.m_val[7] == m_val[7] && v.m_val[8] == m_val[8];
	}
    ...
    Matrix3<T>& operator=(Matrix3sym<T>& M)	{
        m_val[0] = M[0]; m_val[1] = M[1]; m_val[2] = M[2];
        m_val[3] = M[1]; m_val[4] = M[3]; m_val[5] = M[4];
        m_val[6] = M[2]; m_val[7] = M[4]; m_val[8] = M[5];
		return *this;
	}
    ...
    // set identity matrix
    void setIdentity() {
        m_val[0]=1; m_val[1]=0; m_val[2]=0;
        m_val[3]=0; m_val[4]=1; m_val[5]=0;
        m_val[6]=0; m_val[7]=0; m_val[6]=1;
    }   
	...
	{
	    is >> M.m_val[0] >> M.m_val[1] >> M.val[2] >> 
            M.m_val[3] >> M.m_val[4] >> M.val[5] >> 
            M.m_val[6] >> M.m_val[7] >> M.val[8];
	    return is;
	}
...
public:
	...
	template <typename X> Matrix3sym(const X a11, const X a12, const X a13,
                                  const X a22, const X a23,
                                  const X a33) {
        m_val[0]=a11; m_val[1]=a12; m_val[2]=a13;
        m_val[3]=a22; m_val[4]=a23;
        m_val[5]=a33;
    }
    ...
    Vector3<T> calcInvariants() {
        ...
        invar[3] = m_val[0]*(m_val[3]*m_val[5]-m_val[4]*m_val[4]) - m_val[1]*(m_val[1]*m_val[5]-m_val[4]*m_val[2]) + 
