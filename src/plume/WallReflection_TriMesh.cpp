...
/*
    ...
    if ((cellFlag != 0) && (cellFlag != 2)) {
      ...
      Vector3 Xflt = { xPos - disX, yPos - disY, zPos - disZ };
      // vector of the trajectory
      Vector3 Uflt = { disX, disY, disZ };
      // postion of the particle end of trajectory
      Vector3 Xnewflt = Xflt + Uflt;

      Ray test_ray(static_cast<float>(xPos - disX), static_cast<float>(yPos - disY), static_cast<float>(zPos - disZ), Uflt);
      HitRecord hit;
      if (WGD->mesh->triangleBVH->rayHit(test_ray, hit)) {
        Vector3 test = hit.endpt - Xflt;
        std::cout << "----\n";
        std::cout << "hit the mesh at " << hit.endpt << " " << Xflt << " " << Xnewflt << "\n";
        std::cout << "hit dist " << hit.getHitDist() << "/" << Uflt.length() << "=" << hit.getHitDist() / Uflt.length() << "\n";
        std::cout << "hit dist " << test.length() << " " << hit.getHitDist() << "\n";
        std::cout << "hit normal " << hit.n << std::endl;
  ...
  void WallReflection_TriMesh::trajectorySplit_recursive(const WINDSGeneralData *WGD,
                                                         const Plume *plume,
                                                         Vector3Double &X,
                                                         Vector3Double &u,
                                                         const double &d,
...
/*
  ...
  void WallReflection_TriMesh::oneReflection(const WINDSGeneralData *WGD,
                                             const Plume *plume,
                                             Vector3Double &X,
                                             Vector3Double &u,
                                             const double &d,
