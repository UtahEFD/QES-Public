...
void Interpolation::get(particle_array d_particle,
                        const QESWindsData &d_qes_winds_data,
                        ...
                        const int &num_particle)
{
  ...
  interpolate<<<numBlocks, blockSize>>>(num_particle, d_particle, d_qes_winds_data, qes_grid);
  // interpolate_1<<<numBlocks_particle, blockSize>>>(num_particle, d_particle[idx], d_qes_turb_data, qes_grid);
...
void Interpolation::get(particle_array d_new_particle,
                        const QESTurbData &d_qes_turb_data,
                        const QESgrid &qes_grid,
                        const int &num_new_particle)
{
  ...
  interpolate<<<numBlocks, blockSize>>>(num_new_particle,
                                        d_new_particle.pos,
                                        d_new_particle.tau,
