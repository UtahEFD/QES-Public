...
{
  if (storedTimers.find(newTimerName) == storedTimers.end()) {
    //timer not found -> start recording execution time
    storedTimers[newTimerName] = std::chrono::high_resolution_clock::now();
  } else {
    //timer found
...
{
  storedTimers.at(timerName) = std::chrono::high_resolution_clock::now();
  /*
...
void calcTime::printStoredTime(const std::string &timerName)
{
  ...
  */
  std::chrono::duration<double> elapsed = std::chrono::high_resolution_clock::now() - storedTimers.at(timerName);
...
{
  if (storedTimers.find(timerName) == storedTimers.end()) {
    return false;
  } else {
    return true;
  }
...
void calcTime::calcTimeDistribution(const double &totalSeconds,
                                    int &milliseconds,
                                    int &seconds,
                                    int &minutes,
                                    int &hours,
...
void calcTime::printTime(const std::string &timerName,
