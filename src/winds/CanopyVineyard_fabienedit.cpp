...
void CanopyVineyard::nan_count(float nan_var, int &nan_counter, std::string &location)
{
...
// pass d as reference rather than output (faster)
// replace array with structure? (speed? ease?)
// inline function
//float CanopyVineyard::P2L(float P[2], float Lx[2], float Ly[2])
//{
...
void CanopyVineyard::canopyVegetation(WINDSGeneralData *WGD, int building_id)
{
  ...
  int idS;
  int idE;
  // auto (force to be an unsigned int) or size_t instead of int
  // auto id = 0u;
  ...
  for (auto j = 0; j < ny_canopy; j++) {
    for (auto i = 0; i < nx_canopy; i++) {
      ...
      float a_local_bot;
      float a_local_top;
      float a_uw_bot[N_e + 1];
      float a_uw_top[N_e + 1];
      float szt_local_bot;
      float szt_local_top;
      float szt_uw_bot;
      float szt_uw_top;
      float fac_numerator;
      float fac_denominator;
      if (N <= N_e) {
        szt_local_bot = spreadrate_bot * (d_dw * N + d_dw_local) + 0.00001;
        szt_local_top = spreadrate_top * (d_dw * N + d_dw_local) + 0.00001;
      } else {
        szt_local_bot = spreadrate_bot * (d_dw * N_e + d_dw_local) + 0.00001;
        szt_local_top = spreadrate_top * (d_dw * N_e + d_dw_local) + 0.00001;
      }
      a_local_bot = (a_exp * (0.5 * (1 - a_obf) * tanh(1.5 * (szo_bot - z_mid) / szt_local_bot) + 0.5 * (1 + a_obf)) + (1 - a_exp));
      a_uw_bot[0] = 1;
      for (int n = 1; n < N_e + 1; n++) {
        szt_uw_bot = spreadrate_bot * (d_dw * n) + 0.00001;
        a_uw_bot[n] = (a_exp * (0.5 * (1 - a_obf) * tanh(1.5 * (szo_bot - z_rel) / szt_uw_bot) + 0.5 * (1 + a_obf)) + (1 - a_exp));
      }
      a_local_top = (a_exp * (0.5 * (1 - a_obf) * tanh(1.5 * (z_mid - szo_top) / szt_local_top) + 0.5 * (1 + a_obf)) + (1 - a_exp));
      a_uw_top[0] = 1;
      for (int n = 1; n < N_e + 1; n++) {
      ...
      if (N <= N_e) {// i'm in entry region
        fac_numerator = a_local_top;
        for (int n = 0; n <= N; n++) {
          fac_numerator *= a_uw_top[n];
          //       std::cout << " a_uw_top[" << n << "] = " << a_uw_top[n] << "\n";
        }
      } else {// if i'm in equilibrated region
        fac_denominator = a_local_bot;
        for (int n = 0; n <= N_e; n++) {
          fac_denominator *= a_uw_bot[n];
          //       std::cout << " a_uw_bot[" << n << "] = " << a_uw_bot[n] << "\n";
        }
      }
      ...
      fac = fac_numerator / fac_denominator;
      //if (1){
