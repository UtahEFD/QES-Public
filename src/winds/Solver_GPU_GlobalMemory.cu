...
void Solver_GPU_GlobalMemory::solve(const WINDSInputData *WID, WINDSGeneralData *WGD, bool solveWind)
{
  ...
  while ((iter < itermax) && (max_error[0] > tol)) {
    ...
    saveLambdaGlobal<<<numberOfBlocks, numberOfThreadsPerBlock>>>(d_lambda, d_lambda_old, nx, ny, nz);
    cudaCheck(cudaGetLastError());
    int offset = 0;// Red nodes pass
    SOR_RB_Global<<<numberOfBlocks, numberOfThreadsPerBlock>>>(d_lambda, nx, ny, nz, omega, A, B, d_e, d_f, d_g, d_h, d_m, d_n, d_R, offset);
    cudaCheck(cudaGetLastError());
    ...
    calculateErrorGlobal<<<numberOfBlocks, numberOfThreadsPerBlock>>>(d_lambda, d_lambda_old, nx, ny, nz, d_value, d_bvalue, d_error);
