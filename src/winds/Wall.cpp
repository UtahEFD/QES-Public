    ...
    for (auto i = 0; i < nx - 1; i++) {
      for (auto j = 0; j < ny - 1; j++) {
        ...
        if (WGD->icellflag[icell_cent] != 0 && WGD->icellflag[icell_cent] != 2) {
          ...
          // Wall below
          if (WGD->icellflag[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 0 || WGD->icellflag[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 2) {
            WGD->n[icell_cent] = 0.0;
          }
          // Wall above
          if (WGD->icellflag[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 0 || WGD->icellflag[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 2) {
          ...
          // Wall on right
          if (WGD->icellflag[icell_cent - (WGD->domain.nx() - 1)] == 0 || WGD->icellflag[icell_cent - (WGD->domain.nx() - 1)] == 2) {
            if (j > 0) {
              WGD->h[icell_cent] = 0.0;
            }
          }
        ...
        if (WGD->icellflag[icell_cent] == 7) {
          if (WGD->icellflag[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 7 || WGD->icellflag[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 8) {
            WGD->m[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] = WGD->n[icell_cent];
          }

          if (WGD->icellflag[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 7 || WGD->icellflag[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 8) {
            WGD->n[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] = WGD->m[icell_cent];
          }
          ...
          if (WGD->icellflag[icell_cent - (WGD->domain.nx() - 1)] == 7 || WGD->icellflag[icell_cent - (WGD->domain.nx() - 1)] == 8) {
            if (j > 0) {
              WGD->h[icell_cent] = WGD->g[icell_cent - (WGD->domain.nx() - 1)];
            }
          }

          if (WGD->icellflag[icell_cent + (WGD->domain.nx() - 1)] == 7 || WGD->icellflag[icell_cent + (WGD->domain.nx() - 1)] == 8) {
            WGD->g[icell_cent] = WGD->h[icell_cent + (WGD->domain.nx() - 1)];
          }
        }
        ...
        if (WGD->icellflag[icell_cent] == 1) {
          if (WGD->icellflag[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 7 || WGD->icellflag[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 8) {
            WGD->n[icell_cent] = WGD->m[icell_cent - (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)];
          }

          if (WGD->icellflag[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 7 || WGD->icellflag[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)] == 8) {
            WGD->m[icell_cent] = WGD->n[icell_cent + (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)];
          }
...
void Wall::wallLogBC(WINDSGeneralData *WGD, bool isInitial)
{
  ...
  for (auto i = 0u; i < WGD->wall_indices.size(); i++) {
    if (WGD->ni[WGD->wall_indices[i]] == 0.0 && WGD->nj[WGD->wall_indices[i]] == 0.0 && WGD->nk[WGD->wall_indices[i]] == 0.0) {
      int k = WGD->wall_indices[i] / ((WGD->domain.nx() - 1) * (WGD->domain.ny() - 1));
      s_behind = WGD->f[WGD->wall_indices[i]] * (WGD->domain.dy() * WGD->domain.dz_array[k]) * (WGD->domain.dx() * WGD->domain.dx());
      s_front = WGD->e[WGD->wall_indices[i]] * (WGD->domain.dy() * WGD->domain.dz_array[k]) * (WGD->domain.dx() * WGD->domain.dx());
      s_right = WGD->h[WGD->wall_indices[i]] * (WGD->domain.dx() * WGD->domain.dz_array[k]) * (WGD->domain.dy() * WGD->domain.dy());
      s_left = WGD->g[WGD->wall_indices[i]] * (WGD->domain.dx() * WGD->domain.dz_array[k]) * (WGD->domain.dy() * WGD->domain.dy());
      s_below = WGD->n[WGD->wall_indices[i]] * (WGD->domain.dx() * WGD->domain.dy()) * (WGD->domain.dz_array[k] * 0.5 * (WGD->domain.dz_array[k] + WGD->domain.dz_array[k + 1]));
      s_above = WGD->m[WGD->wall_indices[i]] * (WGD->domain.dx() * WGD->domain.dy()) * (WGD->domain.dz_array[k] * 0.5 * (WGD->domain.dz_array[k] + WGD->domain.dz_array[k - 1]));
  ...
  for (auto id = 0u; id < WGD->wall_indices.size(); id++) {
    int k = WGD->wall_indices[id] / ((WGD->domain.nx() - 1) * (WGD->domain.ny() - 1));
    int j = (WGD->wall_indices[id] - k * (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1)) / (WGD->domain.nx() - 1);
    int i = WGD->wall_indices[id] - k * (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1) - j * (WGD->domain.nx() - 1);
    coeff = 1.0;
    count = 0;
    max_dist = sqrt(pow(WGD->domain.dx(), 2.0) + pow(WGD->domain.dy(), 2.0) + pow(WGD->domain.dz_array[k], 2.0));
    ...
    if (WGD->center_id[WGD->wall_indices[id]] == 1 && count == 0) {
      ...
      if (WGD->ni[WGD->wall_indices[id]] >= 0.0) {
        ...
        second_i = std::round((WGD->domain.x[i] - 0.001 + WGD->ni[WGD->wall_indices[id]] * coeff * WGD->domain.dx()) / WGD->domain.dx());
      } else {
        // Finding indices for the second node location in normal to surface direction
        second_i = std::round((WGD->domain.x[i] - 0.001 + WGD->ni[WGD->wall_indices[id]] * coeff * WGD->domain.dx()) / WGD->domain.dx()) - 1;
      }

      if (WGD->nj[WGD->wall_indices[id]] >= 0.0) {
        // Finding indices for the second node location in normal to surface direction
        second_j = std::round((WGD->domain.y[j] - 0.001 + WGD->nj[WGD->wall_indices[id]] * coeff * WGD->domain.dy()) / WGD->domain.dy());
      } else {
        // Finding indices for the second node location in normal to surface direction
        second_j = std::round((WGD->domain.y[j] - 0.001 + WGD->nj[WGD->wall_indices[id]] * coeff * WGD->domain.dy()) / WGD->domain.dy()) - 1;
      }
      ...
      while (condition) {
        ...
        if (WGD->ni[WGD->wall_indices[id]] >= 0.0) {
          ...
          first_i = std::round((WGD->domain.x[i] - 0.001 + WGD->ni[WGD->wall_indices[id]] * coeff * WGD->domain.dx()) / WGD->domain.dx());
          // Finding index for the second node location in normal to surface direction
          second_i = std::round((WGD->domain.x[i] - 0.001 + WGD->ni[WGD->wall_indices[id]] * (coeff + 1) * WGD->domain.dx()) / WGD->domain.dx());
        } else {
          // Finding index for the first node location in normal to surface direction
          first_i = std::round((WGD->domain.x[i] - 0.001 + WGD->ni[WGD->wall_indices[id]] * coeff * WGD->domain.dx()) / WGD->domain.dx()) - 1;
          // Finding index for the second node location in normal to surface direction
          second_i = std::round((WGD->domain.x[i] - 0.001 + WGD->ni[WGD->wall_indices[id]] * (coeff + 1) * WGD->domain.dx()) / WGD->domain.dx()) - 1;
        }

        if (WGD->nj[WGD->wall_indices[id]] >= 0.0) {
          // Finding index for the first node location in normal to surface direction
          first_j = std::round((WGD->domain.y[j] - 0.001 + WGD->nj[WGD->wall_indices[id]] * coeff * WGD->domain.dy()) / WGD->domain.dy());
          // Finding index for the second node location in normal to surface direction
          second_j = std::round((WGD->domain.y[j] - 0.001 + WGD->nj[WGD->wall_indices[id]] * (coeff + 1) * WGD->domain.dy()) / WGD->domain.dy());
        } else {
          // Finding index for the first node location in normal to surface direction
          first_j = std::round((WGD->domain.y[j] - 0.001 + WGD->nj[WGD->wall_indices[id]] * coeff * WGD->domain.dy()) / WGD->domain.dy()) - 1;
          // Finding index for the second node location in normal to surface direction
          second_j = std::round((WGD->domain.y[j] - 0.001 + WGD->nj[WGD->wall_indices[id]] * (coeff + 1) * WGD->domain.dy()) / WGD->domain.dy()) - 1;
        }
        ...
        if (first_i > WGD->domain.nx() - 2 || first_i < 0 || first_j > WGD->domain.ny() - 2 || first_j < 0 || first_k > WGD->domain.nz() - 3 || first_k < 1
            || second_i > WGD->domain.nx() - 2 || second_i < 0 || second_j > WGD->domain.ny() - 2 || second_j < 0 || second_k > WGD->domain.nz() - 3 || second_k < 1) {
          break;
        }
    ...
    first_id = first_i + first_j * WGD->domain.nx() + first_k * WGD->domain.nx() * WGD->domain.ny();
    ...
    second_id = second_i + second_j * WGD->domain.nx() + second_k * WGD->domain.nx() * WGD->domain.ny();
    if (isInitial == true) {
  ...
  dist1 = 0.5 * WGD->domain.dz();
  dist2 = 1.5 * WGD->domain.dz();

  // apply log law fix to the cells with wall below
  for (size_t i = 0; i < WGD->wall_below_indices.size(); i++) {
    ustar_wall = 0.1;// reset default value for velocity gradient
    for (auto iter = 0; iter < 20; iter++) {
      wind_dir = atan2(WGD->v0[WGD->wall_below_indices[i] + WGD->domain.nx() * WGD->domain.ny()], WGD->u0[WGD->wall_below_indices[i] + WGD->domain.nx() * WGD->domain.ny()]);
      vel_mag2 = sqrt(pow(WGD->u0[WGD->wall_below_indices[i] + WGD->domain.nx() * WGD->domain.ny()], 2.0) + pow(WGD->v0[WGD->wall_below_indices[i] + WGD->domain.nx() * WGD->domain.ny()], 2.0));
      vel_mag1 = vel_mag2 - (ustar_wall / WGD->vk) * log(dist2 / dist1);
  ...
  for (size_t i = 0; i < WGD->wall_above_indices.size(); i++) {
    ustar_wall = 0.1;// reset default value for velocity gradient
    for (auto iter = 0; iter < 20; iter++) {
      wind_dir = atan2(WGD->v0[WGD->wall_above_indices[i] - WGD->domain.nx() * WGD->domain.ny()], WGD->u0[WGD->wall_above_indices[i] - WGD->domain.nx() * WGD->domain.ny()]);
      vel_mag2 = sqrt(pow(WGD->u0[WGD->wall_above_indices[i] - WGD->domain.nx() * WGD->domain.ny()], 2.0) + pow(WGD->v0[WGD->wall_above_indices[i] - WGD->domain.nx() * WGD->domain.ny()], 2.0));
      vel_mag1 = vel_mag2 - (ustar_wall / WGD->vk) * log(dist2 / dist1);
  ...
  dist1 = 0.5 * WGD->domain.dx();
  dist2 = 1.5 * WGD->domain.dx();
  ...
  dist1 = 0.5 * WGD->domain.dy();
  dist2 = 1.5 * WGD->domain.dy();

  // apply log law fix to the cells with wall to right
  for (size_t i = 0; i < WGD->wall_right_indices.size(); i++) {
    ustar_wall = 0.1;// reset default value for velocity gradient
    for (auto iter = 0; iter < 20; iter++) {
      wind_dir = atan2(WGD->w0[WGD->wall_right_indices[i] + WGD->domain.nx()], WGD->u0[WGD->wall_right_indices[i] + WGD->domain.nx()]);
      vel_mag2 = sqrt(pow(WGD->u0[WGD->wall_right_indices[i] + WGD->domain.nx()], 2.0) + pow(WGD->w0[WGD->wall_right_indices[i] + WGD->domain.nx()], 2.0));
      vel_mag1 = vel_mag2 - (ustar_wall / WGD->vk) * log(dist2 / dist1);
  ...
  for (size_t i = 0; i < WGD->wall_left_indices.size(); i++) {
    ustar_wall = 0.1;// reset default value for velocity gradient
    for (auto iter = 0; iter < 20; iter++) {
      wind_dir = atan2(WGD->w0[WGD->wall_left_indices[i] - WGD->domain.nx()], WGD->u0[WGD->wall_left_indices[i] - WGD->domain.nx()]);
      vel_mag2 = sqrt(pow(WGD->u0[WGD->wall_left_indices[i] - WGD->domain.nx()], 2.0) + pow(WGD->w0[WGD->wall_left_indices[i] - WGD->domain.nx()], 2.0));
      vel_mag1 = vel_mag2 - (ustar_wall / WGD->vk) * log(dist2 / dist1);
...
{
  for (auto k = 0; k < WGD->domain.nz() - 1; k++) {
    for (auto j = 1; j < WGD->domain.ny() - 1; j++) {
      for (auto i = 1; i < WGD->domain.nx() - 1; i++) {
        int icell_cent = i + j * (WGD->domain.nx() - 1) + k * (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1);
        int icell_face = i + j * WGD->domain.nx() + k * WGD->domain.nx() * WGD->domain.ny();
        if (WGD->icellflag[icell_cent] == 0 || WGD->icellflag[icell_cent] == 2) {
          ...
          WGD->v0[icell_face + WGD->domain.nx()] = 0.0;
          WGD->w0[icell_face] = 0.0;// Set velocity inside the building to zero
          WGD->w0[icell_face + WGD->domain.nx() * WGD->domain.ny()] = 0.0;
        }
      }
    }
  }
...
void Wall::solverCoefficients(WINDSGeneralData *WGD)
{
  ...
  for (auto k = 1; k < WGD->domain.nz() - 2; k++) {
    for (auto j = 0; j < WGD->domain.ny() - 1; j++) {
      for (auto i = 0; i < WGD->domain.nx() - 1; i++) {
        int icell_cent = i + j * (WGD->domain.nx() - 1) + k * (WGD->domain.nx() - 1) * (WGD->domain.ny() - 1);
        WGD->e[icell_cent] = WGD->e[icell_cent] / (WGD->domain.dx() * WGD->domain.dx());
        WGD->f[icell_cent] = WGD->f[icell_cent] / (WGD->domain.dx() * WGD->domain.dx());
        WGD->g[icell_cent] = WGD->g[icell_cent] / (WGD->domain.dy() * WGD->domain.dy());
        WGD->h[icell_cent] = WGD->h[icell_cent] / (WGD->domain.dy() * WGD->domain.dy());
        WGD->m[icell_cent] = WGD->m[icell_cent] / (WGD->domain.dz_array[k] * 0.5 * (WGD->domain.dz_array[k] + WGD->domain.dz_array[k + 1]));
        WGD->n[icell_cent] = WGD->n[icell_cent] / (WGD->domain.dz_array[k] * 0.5 * (WGD->domain.dz_array[k] + WGD->domain.dz_array[k - 1]));
      }
    }
  }
}
