  ...
  if (WID->hrrrInput->interpolationScheme == 1){
	...
	for (auto i = 0; i < nx; i++) {
	  ...
	  if (k + WGD->terrain_face_id[site_id[ii]] -1 > nz - 2) {
	    ...
	    WGD->u0[icell_face] = u_prof[ii * nz + k + WGD->terrain_face_id[site_id[ii]]-1];
	    WGD->v0[icell_face] = v_prof[ii * nz + k + WGD->terrain_face_id[site_id[ii]]-1];
	    WGD->w0[icell_face] = 0.0;
	  }	  
	}
      }
...
void WindProfilerHRRR::biLinearInterpolation(const WINDSInputData *WID, WINDSGeneralData *WGD, int i, int j, int k, int k_mod, int id, int idx, int icell_face, std::vector<int> site_i, std::vector<int> site_j)
{
  ...
  for (auto ii = 0; ii < WGD->closest_site_ids[id].size(); ii++){
    ...
    z_terrain = WGD->domain.z_face[WGD->terrain_face_id[site_id[jj]]];
    
    // If the height difference between the terrain at the curent cell and sensor location is less than ABL height
    if (abs(WGD->domain.z[WGD->terrain_face_id[idx]] - z_terrain) > abl_height[jj]) {
      surf_layer_height = asl_percent * abl_height[jj];
    } else {
      surf_layer_height = asl_percent * (2 * abl_height[jj] - abs(WGD->domain.z[WGD->terrain_face_id[idx]] - z_terrain));
    }
   
    // If sum of z index and the terrain index at the sensor location is outside the domain
    if (k + WGD->terrain_face_id[site_id[jj]] -1 > nz - 2) {
      ...
      v[ii] = v_prof[jj * nz + k + WGD->terrain_face_id[site_id[jj]]];
	     ...
	     && k + WGD->terrain_face_id[site_id[jj]] - 1 < nz
	     && k_mod > k + WGD->terrain_face_id[site_id[jj]]-1) {
      u[ii] = u_prof[jj * nz + k_mod];
      v[ii] = v_prof[jj * nz + k_mod];
    } else {
      u[ii] = u_prof[jj * nz + k + WGD->terrain_face_id[site_id[jj]]-1];
      v[ii] = v_prof[jj * nz + k + WGD->terrain_face_id[site_id[jj]]-1];
    }
  }
