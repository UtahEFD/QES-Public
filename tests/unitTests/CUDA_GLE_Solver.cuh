...
__device__ void solve(particle *p, float par_dt, float invarianceTol, float vel_threshold)
{
  ...
  */
  ...
  mat3 A = { -1.0f + 0.50f * (-p->CoEps * L._11 + L._11 * tau_ddt._11 + L._12 * tau_ddt._12 + L._13 * tau_ddt._13) * par_dt,
             -0.0f + 0.50f * (-p->CoEps * L._12 + L._12 * tau_ddt._11 + L._22 * tau_ddt._12 + L._23 * tau_ddt._13) * par_dt,
             -0.0f + 0.50f * (-p->CoEps * L._13 + L._13 * tau_ddt._11 + L._23 * tau_ddt._12 + L._33 * tau_ddt._13) * par_dt,
             -0.0f + 0.50f * (-p->CoEps * L._12 + L._11 * tau_ddt._12 + L._12 * tau_ddt._22 + L._13 * tau_ddt._23) * par_dt,
             -1.0f + 0.50f * (-p->CoEps * L._22 + L._12 * tau_ddt._12 + L._22 * tau_ddt._22 + L._23 * tau_ddt._23) * par_dt,
             -0.0f + 0.50f * (-p->CoEps * L._23 + L._13 * tau_ddt._12 + L._23 * tau_ddt._22 + L._33 * tau_ddt._23) * par_dt,
             -0.0f + 0.50f * (-p->CoEps * L._13 + L._11 * tau_ddt._13 + L._12 * tau_ddt._23 + L._13 * tau_ddt._33) * par_dt,
             -0.0f + 0.50f * (-p->CoEps * L._23 + L._12 * tau_ddt._13 + L._22 * tau_ddt._23 + L._23 * tau_ddt._33) * par_dt,
             -1.0f + 0.50f * (-p->CoEps * L._33 + L._13 * tau_ddt._13 + L._23 * tau_ddt._23 + L._33 * tau_ddt._33) * par_dt };
  ...
  vec3 b = { -p->velFluct_old._1 - 0.50f * flux_div._1 * par_dt - std::sqrt(p->CoEps * par_dt) * vRandn._1,
             -p->velFluct_old._2 - 0.50f * flux_div._2 * par_dt - std::sqrt(p->CoEps * par_dt) * vRandn._2,
             -p->velFluct_old._3 - 0.50f * flux_div._3 * par_dt - std::sqrt(p->CoEps * par_dt) * vRandn._3 };
  ...
  multiply(A, b, p->velFluct);

  // now check to see if the value is rogue or not
  if (std::abs(p->velFluct._1) >= vel_threshold || isnan(p->velFluct._1)) {
  ...
  if (std::abs(p->velFluct._2) >= vel_threshold || isnan(p->velFluct._2)) {
    // std::cerr << "Particle # " << p->particleID << " is rogue, ";
  ...
  if (std::abs(p->velFluct._3) >= vel_threshold || isnan(p->velFluct._3)) {
    // std::cerr << "Particle # " << p->particleID << " is rogue, ";
