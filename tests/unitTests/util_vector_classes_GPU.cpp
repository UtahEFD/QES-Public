...
TEST_CASE("Matrix Multiplication")
{
  ...
  for (size_t k = 0; k < x_array.size(); ++k) {
    REQUIRE_THAT(x_array[k]._1, Catch::Matchers::WithinRel(6.0f, eps));
    REQUIRE_THAT(x_array[k]._2, Catch::Matchers::WithinRel(15.0f, eps));
    REQUIRE_THAT(x_array[k]._3, Catch::Matchers::WithinRel(24.0F, eps));
  }
}
...
TEST_CASE("Matrix Inversion")
{
  ...
  {
    mat3 B = { 1.0, 2.0, 3.0, 2.0, 1.0, 2.0, 3.0, 2.0, 1.0 };
    ...
    for (size_t k = 0; k < B_array.size(); ++k) {
      REQUIRE_THAT(B_array[k]._11, Catch::Matchers::WithinRel(-0.375f, eps));
      REQUIRE_THAT(B_array[k]._12, Catch::Matchers::WithinRel(0.500f, eps));
      REQUIRE_THAT(B_array[k]._13, Catch::Matchers::WithinRel(0.125f, eps));

      REQUIRE_THAT(B_array[k]._21, Catch::Matchers::WithinRel(0.500f, eps));
      REQUIRE_THAT(B_array[k]._22, Catch::Matchers::WithinRel(-1.000f, eps));
      REQUIRE_THAT(B_array[k]._23, Catch::Matchers::WithinRel(0.500f, eps));

      REQUIRE_THAT(B_array[k]._31, Catch::Matchers::WithinRel(0.125f, eps));
      REQUIRE_THAT(B_array[k]._32, Catch::Matchers::WithinRel(0.500f, eps));
      REQUIRE_THAT(B_array[k]._33, Catch::Matchers::WithinRel(-0.375f, eps));
    }
  }
}
...
TEST_CASE("Symmetric Matrix Invariants")
{
  ...
  for (size_t k = 0; k < x_array.size(); ++k) {
    REQUIRE_THAT(x_array[k]._1, Catch::Matchers::WithinRel(3.0f, eps));
    REQUIRE_THAT(x_array[k]._2, Catch::Matchers::WithinRel(-14.0f, eps));
    REQUIRE_THAT(x_array[k]._3, Catch::Matchers::WithinRel(8.0f, eps));
  }
}
