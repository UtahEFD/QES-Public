...
namespace QUIC
{
	building::building() : type(REGULAR)
	{
		...
		half_lgth = half_wdth = 0.;

		xfo = yfo = zfo = 0;
	
		xco = yco = 0.;
	
		angle gamma = angle(); // ENG
		attenuation = 0;
		/* END from input file. */
	...
	)
	{
		...
		xco = xfo + half_lgth*cos_gamma; // CENTER of building in QUIC domain coordinates
		yco = yfo + half_lgth*sin_gamma;
	...
	void building::print() const
	{
		...
		std::cout << "type: " 	<< type 	<< ", ";
		std::cout << "height: "	<< hght		<< ", ";
		std::cout << "width: " 	<< wdth		<< ", ";
		std::cout << "length: "	<< lgth 	<< ", ";
		std::cout << "xfo: " 		<< xfo		<< ", ";
		std::cout << "yfo: " 		<< yfo		<< ", ";
		std::cout << "zfo: " 		<< zfo		<< ", ";
	...
	bool building::inBuildingQ(int const& i, int const& j, int const& k) const
	{
  ...
  bool building::inBuildingQ(index3D const& loc) const
  {
    return this->inBuildingQ(loc.i, loc.j, loc.k);
  }

	void building::interior
	(
	...
	{		
		...
		float retarding_height = hght;
		float vortex_height    = (upwindtype == 3) ? min((float) bld_prfl, (float) hght) : hght ;
          ...
          else if(doCell && doV)
          {
              ...
              if(ns_flag)
              {
              	vrot = -vhrot	* upwind_velocity_factor;
							...
							ntls.v[gI] = -urot*sin(-gamma.radians()) + vrot*cos(-gamma.radians());
							typs.c [cI] = UPWIND;
						}
	...
	void building::rooftop
	(
		celltypes typs, velocities ntls,
		float const& dx, float const& dy, float const& dz
	)
	{		
		angle tol               = angle(30., DEG);
		bool perpendicular_flag = upwind_rel.isCardinalQ(tol);
		bool ns_flag            = upwind_rel.isPoleQ    (tol);
		...
		// Basic ENG angles.
		angle a_000 = angle(  0., DEG);
		angle a_015 = angle( 15., DEG);
		angle a_045 = angle( 45., DEG);
		angle a_090 = angle( 90., DEG);
		angle a_135 = angle(135., DEG);
		angle a_180 = angle(180., DEG);
		
		float xfront = (upwind_rel.inDirQ( WESTNORTHWEST, a_090)) ? half_lgth : -half_lgth ;
		float yfront = (upwind_rel.inDirQ(SOUTHSOUTHWEST, a_090)) ? half_wdth : -half_wdth ;
		
		angle xnorm = (upwind_rel.inDirQ(NORTHEAST, a_015)) ? gamma + a_180 :
									(upwind_rel.inDirQ(SOUTHEAST, a_015)) ? gamma - a_180 :
									(upwind_rel.inDirQ(SOUTHWEST, a_015)) ? gamma : 
									(upwind_rel.inDirQ(NORTHWEST, a_015)) ? gamma :
																													a_000 ;
																																		
		angle ynorm = (upwind_rel.inDirQ(NORTHEAST, a_015)) ? gamma - a_090 :
									(upwind_rel.inDirQ(SOUTHEAST, a_015)) ? gamma + a_090 :
									(upwind_rel.inDirQ(SOUTHWEST, a_015)) ? gamma + a_090 : 
									(upwind_rel.inDirQ(NORTHWEST, a_015)) ? gamma - a_090 :
																												  a_000 ;

		angle roofangle = (upwind_rel.inDirQ(NORTHEAST, a_015)) ? calculateRoofAngle( a_000,  a_045) :
											(upwind_rel.inDirQ(SOUTHEAST, a_015)) ? calculateRoofAngle( a_000, -a_045) :
											(upwind_rel.inDirQ(SOUTHWEST, a_015)) ? calculateRoofAngle(-a_090, -a_135) : 
											(upwind_rel.inDirQ(NORTHWEST, a_015)) ? calculateRoofAngle( a_090,  a_135) :
																															a_000 ;
		...
		int k_ref = kend + int(.5*hght/dz);		
		bool in_subdomain = (k_ref > typs.dim.z) ? false : this->inSubdomainQ() && upwindtype > 0 ;
	
		if(in_subdomain)
		{
			float Bs = min(Weff, (float) hght);
			float BL = max(Weff, (float) hght);
			...
			for(int k = kend + 1; k <= k_ref; k++)
			{
			    ...
			    // Check to see if velocity vector is above the building or in a street canyon cell					
			  	...
			  	bool wflag = NotCanyonCell &&  k_end_isBuild;
 					...
 					determineVelocityLocations(i, j, dx, dy, gamma, x_u, x_v, x_w, y_u, y_v, y_w);
	...
	) // const for effective dims ???
	{
		...
		if(rooftype == 2)
		{
			bool roof_perpendicular = upwind_rel.isCardinalQ(angle(30., DEG));
			bool ns_flag            = upwind_rel.isPoleQ    (angle(30., DEG));
		...
		{			
			...
			for(int k = zfo + rnd(eff_height/dz); k >= kstart; k--)
			{
			    ...
			    for(int x_idx = 1; x_idx <= int(Lr/dxy) + 1; x_idx++)
			    {
			      ...
			      if(i >= typs.dim.x - 1 || i <= 0 || j >= typs.dim.y - 1 || j <= 0) {break;}
	...
	void building::canyon
	(
		celltypes typs, velocities const& ntls, std::vector<building*> const& bldngs,
		float const& dx, float const& dy,	float const& dz
	) const
	{
		float dxy = min(dx,dy);
		...
		angle upwind_norm  = (upwind_rel.inDirQ(NORTH, crdnl_tol)) ? gamma + a_090 :
												 (upwind_rel.inDirQ(EAST,  crdnl_tol)) ? gamma         :
												 (upwind_rel.inDirQ(SOUTH, crdnl_tol)) ? gamma - a_090 :
												 																				 gamma - a_180 ;
		
		angle upwind_norm1 = (upwind_rel.inDirQ(NORTHEAST, ordnl_tol)) ? gamma + a_090 :
									 			 (upwind_rel.inDirQ(SOUTHEAST, ordnl_tol)) ? gamma         :
									 			 (upwind_rel.inDirQ(SOUTHWEST, ordnl_tol)) ? gamma - a_090 :
									 			 (upwind_rel.inDirQ(NORTHWEST, ordnl_tol)) ? gamma + a_180 :
																																  			     a_000 ;
         
    angle upwind_norm2 = (upwind_rel.inDirQ(NORTHEAST, ordnl_tol)) ? gamma         :
									 			 (upwind_rel.inDirQ(SOUTHEAST, ordnl_tol)) ? gamma - a_090 :
									 			 (upwind_rel.inDirQ(SOUTHWEST, ordnl_tol)) ? gamma - a_180 :
									 			 (upwind_rel.inDirQ(NORTHWEST, ordnl_tol)) ? gamma + a_090 :
																																    			   a_000 ;
		int cell_row = typs.dim.x;
		int cell_slc = typs.dim.y*typs.dim.x;
		int grid_row = ntls.dim.x;
		...
		angle along_dir  = a_000;
		angle cross_dir  = a_000;
		angle canyon_dir = a_000;
		float velmag     = 0.;
		int   k_ref      = 0;
      ...
      for(int k = kend; k >= kstart; k--)
      {
        ...
        for(int x_idx = 0; x_idx <= 2*int(Lr/dxy) + 1; x_idx++)
        {
          ...
          if(i >= ntls.dim.x - 1 || i <= 0 || j >= ntls.dim.y - 1 || j <= 0) {break;}
          ...
          if(typs.c[cI] == SOLID && x_idx_min >= 0)
          {
          	...
          	x_idx_max   = x_idx - 1;
						...
						{						
							...
							{				
								...
								else if(typs.c[cndx_c_kref - 1] != SOLID)
								{
									...
									canyon_dir = (ntls.u[vndx_c_kref] > 0.) ? a_000 :  a_180 ;
								}	
								else
								{
									velmag     = fabs(ntls.v[vndx_c_kref]);
									canyon_dir = (ntls.v[vndx_c_kref] > 0.) ? a_090 : -a_090 ;
								}
								top_flag = true;
							}
							else
						...
						// Find the along canyon and cross canyon directions
							building* dwnwnd_bldng = findBuilding(bldngs, i, j, k);
						
							if(dwnwnd_bldng->hght < hght)
							{
								canyon_flag = top_flag = false;
								S = 0.;
								break;
	...
	void building::calculatePhiPrime(velocities const& ntls, float const& dx, float const& dy, float const& dz)
	{
		...
		int xndx = rnd((xfo + lgth/2.) / dx) + 1; // \\todo check these. Should they all lose 1.?
		int yndx = rnd(yfo  / dy) + 1;
		int zndx = rnd(hght / dz) + 2;
		...
		angle a_360 = angle(360., DEG, MET);		
		
		angle theta = a_000; // MET
		
		// Is there a better way to do this?
		if(cu == 0 && cv >  0) {theta = a_180;}
		if(cu == 0 && cv <= 0) {theta = a_000;}
		
		// Temp angles are in engineering, but everything works out.
		if(cu <  0 && cv <  0) {theta = a_090 - angle(atan( cv/cu));}
		if(cu <  0 && cv >= 0) {theta = a_090 + angle(atan(-cv/cu));}
		if(cu >  0 && cv >= 0) {theta = a_270 - angle(atan( cv/cu));}
		if(cu >  0 && cv <  0) {theta = a_270 + angle(atan(-cv/cu));}
		
		// Is there a better way to do this?		
		angle phi = (theta > a_270) ? theta - a_270 : -theta + a_270 ;
		
		phiprime  =	(phi < a_090) ? ( phi - a_000) :
								(phi < a_180) ? (-phi + a_180) :
								(phi < a_270) ? ( phi - a_180) :
																(-phi + a_360) ;
	}

	void building::calculateDimensionalIndices(float const& dx, float const& dy, float const& dz)
	{
		...
		istart = rnd(xfo / dx);
		iend   = istart + int(lgth / dx);
				
		jstart = rnd((yfo - half_wdth) / dy);
		jend   = jstart + int(wdth / dy);
				
		kstart = rnd(zfo / dz) + 1;
		kend   = kstart + int(hght / dz) - 1; // Actually one less than terminating index.
	...
	void building::calculateFootprintCorners()
	{
		// Location of corners relative to the center of the building.
	
		// Which corner is this?
		x1 = xfo + half_wdth*sin_gamma - xco;
		y1 = yfo - half_wdth*cos_gamma - yco;
		
		// Which corner is this?
		x2 = x1 + lgth*cos_gamma;
		y2 = y1 + lgth*sin_gamma;
		
		// Which corner is this?
		x4 = xfo - half_wdth*sin_gamma - xco;
		y4 = yfo + half_wdth*cos_gamma - yco;		
		
		// Which corner is this? // Leave last.
		x3 = x4 + lgth*cos_gamma;
		y3 = y4 + lgth*sin_gamma;
	}

	void building::calculateUpwindDirection
	(
		velocities const& ntls, 
		float const& dx, 
		float const& dy, 
	...
	void building::calculateUpwindCorners()
	{
		angle tol = angle(10., DEG); // ENG
	...
	void building::calculateWakeCorners()
	{
	...
	void building::calculateWeffLeffLr()
	{
		// Potentially modify the Effective length based on wind angle.

		angle upwind_rel_norm = upwind_rel + angle(90., DEG);
		angle beta            = angle(fabs(atan2(lgth, wdth)));
		...
		{
			Leff = fabs(wdth/sin(upwind_rel));
		}
		else
		{
			Leff = fabs(lgth/cos(upwind_rel));
		}
		...
		if(waketype == 2)
		{
			...
			{
				Weff = fabs(wdth/sin(upwind_rel_norm));
			}
			else
			{
				Weff = fabs(lgth/cos(upwind_rel_norm));
			}
		}
		else
		{
			float fabs_phiprime_gamma = fabs(phiprime.radians() - gamma.radians());
			Weff = lgth*sin(fabs_phiprime_gamma) + wdth*cos(fabs_phiprime_gamma);
		}
		this->calculateLr(hght);
	}

	void building::calculateLr(float const& _hght)
	{
		// Length over Height and Width over Height ratios.
		float LoverH = Leff / _hght;
		float WoverH = Weff / _hght;
		
		if(LoverH > 3.)  {LoverH = 3.;}
		if(LoverH < .3)  {LoverH = .3;}
		if(WoverH > 10.) {WoverH = 10.;}
		
		Lr = (1.8*_hght*WoverH) / (pow(LoverH, .3)*(1. + .24*WoverH));		
	}

	bool building::inSubdomainQ() const
	{
		bool n_sbdmn = false;
	...
	void building::determineVelocityLocations
	(
	...
	void building::determineCanyonDirections
	(
	...
	) const
	{
		...
		*/
		if(a_090 - beta <= thetad && thetad <= a_090 + beta)
		{
			if(downwind_rel <= a_000)
			{
				if(downwind_rel <= -a_090)
				{
					along_dir = -a_180 + gamma;
					cross_dir = along_dir - a_090;
				}
				else
				{
					along_dir = gamma;
	 	...
	 	else if(-a_090 + beta < thetad && thetad < a_090 - beta)
	 	{
			if(-downwind_rel <= -a_090 || a_090 <= downwind_rel) //fabs(downwind_rel) >= a_090)
			{
				if(downwind_rel < a_000)
				{
					along_dir = -a_090 + gamma;
					cross_dir = along_dir + a_090;
				}
				else
				{
		...
		else if(thetad <= -a_090 + beta && thetad >= -a_090 - beta)
		{
			if(downwind_rel >= a_000)
			{
				if(downwind_rel <= a_090)
		...
		{
			if(-a_090 < downwind_rel || downwind_rel < a_090) //fabs(downwind_rel) < a_090)
			{
				if(downwind_rel >= a_000)
				{
					along_dir = a_090 + gamma;
	...
	void building::searchUpwindForRooftopDisruptor
	(
		celltypes typs, 
		float const& dx, float const& dy
	)
    ...
    for(int y_idx = 1; y_idx < 2*int((yw1 - yw3)/dxy) - 1; y_idx++)
    {
			...
			for(int x_idx = int(Lr/dxy) + 1; x_idx >= 1; x_idx--)
			{
				...
				if(1 <= i && i <= typs.dim.x && 1 <= j && j <= typs.dim.y)
				{
	...
	angle building::calculateRoofAngle(angle const& a1, angle const& a2) const
	{		
		return 
			angle
			(
	...
	void sortBuildings(std::vector<building*> bldngs)
	{
		int veggies = 0;
		for(unsigned int j = 0; j < bldngs.size(); j++)
		{
			...
			{
				if(bldngs[i]->type == building::VEGETATION)
				{
					swapBuildings(bldngs[i], bldngs[j]);
					veggies++;				
				}
				else if(bldngs[i]->hght > bldngs[j]->hght)
				{
					swapBuildings(bldngs[i], bldngs[j]);
				}
			}
