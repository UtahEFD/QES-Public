...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
         ...
         if(bldtype(ibuild) .eq. 4 .and. atten(ibuild) .lt. 0.)then
         ...
         if((bldtype(ibuild) .eq. 1 .or. bldtype(ibuild) .eq. 10) .and. roofflag .eq. 2)then
            ...
            !print*,'nupwind = ',nupwind
            !print*,'int((yw1 - yw3)/dxy) = ',int((yw1 - yw3)/dxy)
...
lp002:         do y_idx=1,2*int((yw1-yw3)/dxy)-1
                  ...
                  if(dN .ne. dN)print*,i,j,k,zb,eff_height,Lr(ibuild)
                  x_idx_min=-1
...
! Cavity             
                           ...
                           !uo(iu,ju,k)=-uo_h*min(((cav_fac*Lr(ibuild)-xc)/(cav_fac*Lr(ibuild)))**2.,1.)
                           uo(iu,ju,k)=-uo_h*min((1.-xc/(cav_fac*dN))**2.,1.)*min(sqrt(1.-abs(yc/ynorm)),1.)
...
! Cavity
                           ...
                           !vo(iv,jv,k)=-vo_h*min(((cav_fac*Lr(ibuild)-xc)/(cav_fac*Lr(ibuild)))**2.,1.)
                           vo(iv,jv,k)=-vo_h*min((1.-xc/(cav_fac*dN))**2.,1.)*min(sqrt(1.-abs(yc/ynorm)),1.)
